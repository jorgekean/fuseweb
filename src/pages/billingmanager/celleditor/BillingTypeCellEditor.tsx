...
interface BillingTypeCellProps {
    ...
    options: any[]; // The array of project options for the select
}

const BillingTypeCellEditor: React.FC<BillingTypeCellProps> = ({
    row,
    column,
    updateData,
    options,
    ...
    const [selectedBillingType, setSelectedBillingType] = useState(row.original.billingType);
    ...
    useEffect(() => {
        setSelectedBillingType(row.original.billingType);
    ...
    const handleSelectChange = (selectedData: any) => {
        // newValue is the full option object selected from react-select        
        setSelectedBillingType(selectedData.value); // Update local state with the selected object
        // Call the updateData function with the row's unique ID and the full new value object
        updateData(row.original.id, column.id, selectedData.value);
        setIsEditing(false); // Exit edit mode after selection
    };

    const handleDoubleClick = () => {
        console.log(selectedBillingType, options)
        setIsEditing(true);
    };
    ...
    const currentSelectValue = selectedBillingType ?
        formattedOptions.find(option => option.value === selectedBillingType) : null;
        ...
        <div className="relative group">
            <span
                ...
                className={classNames(
                    ...
                    `${row.original.billingType?.toLowerCase() == "billable" ? "bg-primary4 hover:bg-primary5" : row.original.billingType?.toLowerCase() == "nonbillable" ? "bg-secondary4 hover:bg-secondary5" : ""} 
                    bg-opacity-20  dark:bg-primary4 dark:hover:bg-primary5`,
                    `cursor-pointer`
                )}
            >
                {selectedBillingType}
...
export default BillingTypeCellEditor;